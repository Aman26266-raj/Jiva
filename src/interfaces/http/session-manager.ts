...
export interface SessionIdentityOptions {
  ...
  contactId?: string;
  /** Transport/channel that originated the conversation. */
  channel?: string;
}
...
export class SessionManager extends EventEmitter {
  ...
  async getOrCreateSession(
    ...
    const creationPromise = this.createSession(tenantId, sessionId, codeModeEnabled, runtimeConfigUri, identity, agentConfig);
    ...
    try {
      ...
      if (agentSession) {
        ...
        const reasoningModel = createModelClient({
          ...
          apiKey: rm.apiKey || '',
          model: agentConfig?.model ?? rm.model ?? rm.defaultModel,
          type: 'reasoning',
      ...
      if (agentSession) {
        ...
        // Inline agentConfig remains supported for current-main clients. When
        // supplied it replaces stored/default MCP configuration for this session.
      ...
      if (codeModeEnabled) {
        ...
        agent = new CodeAgent({
          ...
          workspace,
          conversationManager,
          maxIterations: agentSession?.maxIterations ?? maxIterations,
          lspEnabled,
  ...
  async chatTurn(
    ...
    const agent = await this.getOrCreateSession(tenantId, sessionId, runtimeConfigUri, identity, agentConfig);
